...
class TLVParser:
    ...
    def _parse_frame(self, data: bytes, frame_num: int,
        ...
        for _ in range(num_tlvs):
            ...
            if tlv_type == self.TLV_DETECTED_POINTS:
            ...
            elif tlv_type == self.TLV_NOISE_PROFILE:
                frame.noise_profile = self._parse_range_profile(payload)
            ...
            elif tlv_type == self.TLV_STATISTICS:
                frame.statistics = self._parse_statistics(payload)
                ...
                frame.temperature_stats = self._parse_temperature_stats(payload)
    ...
    def _parse_side_info(self, payload: bytes) -> list[tuple[float, float]]:
        # int16 SNR (0.1 dB units), int16 noise (0.1 dB units) per point
        result = []
        for i in range(0, len(payload) - 3, 4):
            snr, noise = struct.unpack_from('<HH', payload, i)
            result.append((snr * 0.1, noise * 0.1))
        return result

    def _parse_statistics(self, payload: bytes) -> dict:
        if len(payload) < 24:
            return {}
        vals = struct.unpack_from('<6I', payload)
        return {
            'interFrameProcessingTime_us': vals[0],
            'transmitOutputTime_us':       vals[1],
            'interFrameProcessingMargin_us': vals[2],
            'interChirpProcessingMargin_us': vals[3],
            'activeFrameCPULoad_pct':      vals[4],
            'interFrameCPULoad_pct':       vals[5],
        }

    def _parse_temperature_stats(self, payload: bytes) -> dict:
        if len(payload) < 28:
            return {}
        (valid, time_ms,
         rx0, rx1, rx2, rx3,
         tx0, tx1, tx2,
         pm, dig0, dig1) = struct.unpack_from('<II HHHH HHH HHH', payload)
        return {
            'tempReportValid': valid,
            'time_ms': time_ms,
            'tmpRx0_C': rx0, 'tmpRx1_C': rx1, 'tmpRx2_C': rx2, 'tmpRx3_C': rx3,
            'tmpTx0_C': tx0, 'tmpTx1_C': tx1, 'tmpTx2_C': tx2,
            'tmpPm_C':  pm,  'tmpDig0_C': dig0, 'tmpDig1_C': dig1,
        }
