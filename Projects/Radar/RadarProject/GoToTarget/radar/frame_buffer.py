...
class FrameBuffer:
    def __init__(self, maxlen: int = BUFFER_SIZE) -> None:
        ...
        self._noise_prof: deque[np.ndarray]          = deque(maxlen=maxlen)
    ...
    def push(self, frame: RadarFrame) -> None:
        ...
        if frame.noise_profile is not None:
            self._noise_prof.append(frame.noise_profile)
    ...
    @property
    def latest_noise_profile(self) -> Optional[np.ndarray]:
        return self._noise_prof[-1] if self._noise_prof else None
    ...
    def all_noise_profiles(self) -> List[np.ndarray]:
        return list(self._noise_prof)
